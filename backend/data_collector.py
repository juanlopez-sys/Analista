# ============================================================
# data_collector.py — Indicadores técnicos y datos locales
# Binance DESACTIVADO — los datos se importan via /api/import-db
# ============================================================

import requests  # Se mantiene por si acaso, pero no se usa para precios
import pandas as pd
...
# ============================================================
# RECOLECCIÓN PRINCIPAL — DESACTIVADA (datos vienen del .db)
# ============================================================

def collect_all(verbose: bool = True) -> dict:
    """
    Descarga desde Binance DESACTIVADA — los datos se importan
    subiendo el archivo .db desde el frontend (/api/import-db).
    Esta función retorna vacío para no romper código que la llame.
    """
    logger.info("collect_all() desactivado — usar /api/import-db para subir datos")
    return {crypto: {tf: 0 for tf in TIMEFRAMES} for crypto in ACTIVE_CRYPTOS}


def get_current_price(crypto: str) -> float:
    """
    Obtiene el último precio conocido desde la base de datos local
    (última vela importada), sin llamar a Binance.
    """
    try:
        conn   = get_connection()
        cursor = conn.cursor()
        # Buscar en todos los timeframes disponibles, priorizar el más granular
        for tf in ["1m", "5m", "15m", "30m", "1h", "4h", "1d"]:
            table = f"candles_{crypto.lower()}_{tf}"
            try:
                cursor.execute(f"SELECT close FROM {table} ORDER BY timestamp DESC LIMIT 1")
                row = cursor.fetchone()
                if row:
                    conn.close()
                    return float(dict(row)["close"])
            except Exception:
                continue
        conn.close()
        return 0.0
    except Exception as e:
        logger.warning(f"Error obteniendo precio local de {crypto}: {e}")
        return 0.0
