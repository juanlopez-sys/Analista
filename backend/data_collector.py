...
def collect_all(verbose: bool = True) -> dict:
    """Desactivado — datos se importan via /api/import-db."""
    return {crypto: {tf: 0 for tf in TIMEFRAMES} for crypto in ACTIVE_CRYPTOS}


def get_current_price(crypto: str) -> float:
    """Obtiene el último precio desde la BD local, sin llamar a Binance."""
    try:
        conn   = get_connection()
        cursor = conn.cursor()
...
def get_all_prices() -> dict:
    return {crypto: get_current_price(crypto) for crypto in ACTIVE_CRYPTOS}
