...
async def update_candles(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    return {
        "ok": False,
        "message": "La descarga desde Binance está desactivada. Sube tu archivo .db desde 'Importar datos'."
    }
...
async def update_data(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        return {"ok": True, "total_candles": 0, "total_news": total_news,
                "note": "Velas no actualizadas — usa /api/import-db para subir datos."}
    except Exception as e:
...
async def import_db_progress(
    ...
    logger.info(f"SSE import: {file.filename} ({len(content)/1024/1024:.1f} MB)")
    ...
    def event_stream():
        ...
        try:
            ...
            for i, table in enumerate(tables, 1):
                ...
                if result["error"]:
                    errores.append(f"{table}: {result['error']}")
                else:
            ...
            logger.error(f"Error en SSE import: {e}")
