...
async def update_candles(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    return {
        "ok": False,
        "message": "La descarga desde Binance está desactivada en este entorno. "
                   "Sube tu archivo .db local desde la sección 'Importar datos'."
    }
...
async def update_data(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        return {
            ...
            "note": "Velas no actualizadas — usa /api/import-db para subir datos de precios."
        }
    except Exception as e:
